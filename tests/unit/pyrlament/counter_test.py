...
class TestSeatsCounter(unittest.TestCase):
    ...
    def test_district_support_votes(self):
        ...
        for i in range(0, len(election.districts)):
            yyy = []
            given = election.districts[i]
            for c in given.candidates_votes:
                yyy.append([c.party_name, c.votes])
            xxx.append(yyy)
